...
class ModelHandler:
    ...
    def label_encode_target(self):
        encode_map = {"Canceled": 0, "Not_Canceled": 1}
        self.output_data = self.output_data.replace(encode_map)
        with open('booking_encode.pkl', 'wb') as f:
            joblib.dump(encode_map, f)
        self.target_encoder = encode_map
    ...
    def save_model_to_file(self):
        joblib.dump(self.model, open('XGB_booking.pkl', 'wb'))
        joblib.dump(self.target_encoder, open('booking_encode.pkl', 'wb'))
        joblib.dump(self.meal_encoder, open('oneHot_encode_type.pkl', 'wb'))
        joblib.dump(self.room_encoder, open('oneHot_encode_room.pkl', 'wb'))
        joblib.dump(self.market_encoder, open('oneHot_encode_market.pkl', 'wb'))

# Kode utama tetap sama
